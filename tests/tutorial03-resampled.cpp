...
static int audio_resampling(AVCodecContext *audio_decode_ctx, AVFrame *audio_decode_frame,
...
int main(int argc, char *argv[]) {
    ...
    SDL_Window *screen = SDL_CreateWindow(
        "FFmpeg SDL Video Player", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
        pCodecCtx->width / 2, pCodecCtx->height / 2, SDL_WINDOW_OPENGL | SDL_WINDOW_ALLOW_HIGHDPI);
    ...
    int numBytes;
    numBytes =
        av_image_get_buffer_size(AV_PIX_FMT_YUV420P, pCodecCtx->width, pCodecCtx->height, 32);
    ...
    // The fields of the given image are filled in by using the buffer which points to the image
    // data buffer.
            ...
            while (ret >= 0) {
                ...
                if (++frameIndex <= maxFramesToDecode) {
                    ...
                    printf(
                        ...
                        av_get_picture_type_char(pFrame->pict_type), pCodecCtx->frame_number,
                        pFrame->pts, pFrame->pkt_dts, pFrame->key_frame,
    ...
    for (;;) {
        ...
        while (audio_pkt_size > 0) {
            ...
            if (got_frame) {
                // audio resampling
                data_size =
                    audio_resampling(aCodecCtx, avFrame, AV_SAMPLE_FMT_S16, aCodecCtx->channels,
