...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0;
        ...
        if (local_codec_params->codec_type ==
            AVMEDIA_TYPE_VIDEO) {
            printf(
                "[视频] 编解码器: %s; 分辨率: "
                "%dx%d\n",
                local_codec->name,
                local_codec_params->width,
                local_codec_params->height);
            codec_params = local_codec_params;
            codec = local_codec;
            video_stream_index = i;
        } else if (local_codec_params
                       ->codec_type ==
                   AVMEDIA_TYPE_AUDIO) {
            printf(
                "[音频] 编解码器: %s; 通道数: "
    ...
    SwsContext *img_convert_context =
        sws_getContext(codec_context->width,
                       codec_context->height,
    ...
    int i = 0;  //
    while (av_read_frame(format_context,
                         packet) >= 0) {
        if (packet->stream_index ==
            video_stream_index) {
            int ret = avcodec_send_packet(
                codec_context, packet);
            if (ret < 0) {
                printf(
